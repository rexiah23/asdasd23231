...
import { DollarSign, ArrowLeft, User, Mail, Phone, FileText, AlertTriangle, FileCheck, Shield } from 'lucide-react';
...
function CustomerInfoForm({ onSubmit }: { onSubmit: (data: { name: string; email: string; phone: string; promotionalConsent: boolean }) => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ name, email, phone, promotionalConsent });
  };
        ...
        <div className="flex items-start mt-4">
          <div className="flex items-center h-5">
            <input
              ...
              className="h-4 w-4 text-green-500 focus:ring-green-500 border-gray-600 rounded bg-zinc-800"
            />
          </div>
          <div className="ml-3 text-sm">
            <label htmlFor="promotional-consent" className="text-gray-300">
              I agree to receive promotional emails from SG Supercars about new vehicles, special offers, and updates. I understand I can unsubscribe at any time.
            </label>
          </div>
        </div>
      </div>
...
function CheckoutForm({ carId, customerInfo }: { carId: string; customerInfo: { name: string; email: string; phone: string; promotionalConsent: boolean } }) {
...
export function PaymentPage({ carId: propCarId }: PaymentPageProps) {
  ...
  const [customerInfo, setCustomerInfo] = useState<{ name: string; email: string; phone: string; promotionalConsent: boolean } | null>(null);
  ...
  const createPaymentIntent = async (customerInfo: { name: string; email: string; phone: string; promotionalConsent: boolean }) => {
    if (!car) return;
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/stripe/create-payment-intent`,
        {
          ...
          body: JSON.stringify({ 
            ...
            email: customerInfo.email,
            phone: customerInfo.phone,
            promotionalConsent: customerInfo.promotionalConsent,
            metadata: {
              car_id: carId || '',
              order_type: 'deposit'
            }
  ...
  const handleCustomerInfoSubmit = (info: { name: string; email: string; phone: string; promotionalConsent: boolean }) => {
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-zinc-800 rounded-lg">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-300">Fully Refundable Deposit</p>
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2 text-gray-300">
                <li className="flex items-center gap-2">
                  ...
                  <span>In-house inspector's detailed interior & exterior review</span>
                </li>
                <li className="flex items-center gap-2">
                  <FileCheck size={16} className="text-green-400" />
                  <span>Official Encar.com independent inspection report (If Available)</span>
                </li>
                <li className="flex items-center gap-2">
                  <FileCheck size={16} className="text-green-400" />
                  <span>KOTSA (Korean Transportation Safety Authority) inspection</span>
                </li>
                <li className="flex items-center gap-2">
                  <FileCheck size={16} className="text-green-400" />
                  <span>High-resolution photos and videos of the vehicle</span>
                </li>
                <li className="flex items-center gap-2">
                  <FileCheck size={16} className="text-green-400" />
                  <span>Complete insurance and registration history check</span>
                </li>
                <li className="flex items-center gap-2">
                  <Shield size={16} className="text-green-400" />
                  <span>Fully refundable deposit until purchase confirmation</span>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-zinc-800 rounded-lg p-6 border border-zinc-700">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Vehicle Purchase and Import Brokerage Agreement Summary</h3>
              </div>
              
              <div className="space-y-4 text-gray-300">
                <div className="space-y-2">
                  <h4 className="font-medium text-white">Parties and Purpose</h4>
                  <p className="text-sm">This agreement is between SG Supercars Inc. (the "Broker") and you (the "Client") for the purchase and import of a vehicle.</p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-medium text-white">Deposit (Fully Refundable)</h4>
                  <p className="text-sm">The $1,000 CAD deposit is fully refundable before a final purchase commitment is made.</p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-medium text-white">Purchase Price and Payment Terms</h4>
                  <p className="text-sm">
                    The total purchase price includes:
                    <ul className="mt-2 ml-4 space-y-1">
                      <li>• Vehicle cost</li>
                      <li>• Overseas shipping (RORO or container)</li>
                      <li>• Import duties (6.1%)</li>
                      <li>• Customs clearance fees</li>
                      <li>• GST (5%)</li>
                      <li>• Inland shipping within Canada (If applicable)</li>
                      <li>• Brokerage service fees</li>
                      <li>• All other applicable government fees for importing the vehicle</li>
                    </ul>
                    The remaining balance is due after we confirm the vehicle's availability and disclose all available condition records to you, and you confirm your desire to proceed with the purchase.
                  </p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-medium text-white">Vehicle Condition and Disclaimers</h4>
                  <p className="text-sm">The vehicle is sold "as-is, where-is" with no warranty. The Broker is not responsible for the vehicle's condition, history, or any issues discovered after purchase.</p>
                </div>

                <div className="space-y-2">
                  <h4 className="font-medium text-white">Refundable Deposit / Cancellation Rights</h4>
                  <p className="text-sm">You have the right to cancel this agreement and receive a full refund of your deposit before the final purchase commitment is made.</p>
                </div>

                <div className="mt-4 p-3 bg-zinc-800/50 border border-zinc-600 rounded-md">
                  <p className="text-sm text-white">
                    By placing this deposit, you acknowledge that you have read, understood, and agree to these terms and conditions, including all fees, shipping arrangements, and import procedures outlined above.
                  </p>
                </div>
              </div>
            </div>
